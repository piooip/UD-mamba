...
parser.add_argument('--max_iterations', type=int,
                    default=1000000, help='maximum epoch number to train')
...
def calculate_entropy(predictions):
    ...
    predictions = predictions + 1e-10  # 避免 log(0)
    probabilities = predictions / predictions.sum(axis=0, keepdims=True)  # 按高度和宽度求和
    entropy = -np.sum(probabilities * np.log(probabilities), axis=0)  # 对每个像素计算熵
    return entropy
...
def validate_WSI(model, val_loader, overlap=0.25, save_folder=None, save_csv=None):
    ...
    #hd_metric = HausdorffDistanceMetric(distance_metric="euclidean")
    metric_list = []
    D=[]
    with torch.no_grad():
        for i_batch, sampled_batch in enumerate(val_loader):
            val_images, val_labels =sampled_batch["image"].cuda(), sampled_batch["label"].cuda()
            #val_labels = val_labels.unsqueeze(0)
            val_images, val_labels = val_images.float(), val_labels.long()
            val_outputs = sliding_window_inference(val_images, [256,256], 4, model, overlap=overlap)
            preds = val_outputs
            preds=preds.cuda()
            val_labels=val_labels.cuda()
            dice_metric.add_batch(preds, val_labels)
            preds = torch.argmax(torch.softmax(
            preds, dim=1), dim=1).squeeze(0)
            val_labels=val_labels.squeeze(0)
            preds=preds.cpu().numpy()
            val_labels=val_labels.cpu().numpy()
            m=calculate_metric_percase(preds, val_labels)
            metric_list.append(m)
            print(m)
            D.append(m)
            #hd_metric.add_batch(preds, val_labels)
...
def train(args, snapshot_path):
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            volume_batch, label_batch = volume_batch.float(), label_batch.long()
...
if __name__ == "__main__":
    ...
    snapshot_path = "../ACDC/{}_{}_labeled/{}".format(
